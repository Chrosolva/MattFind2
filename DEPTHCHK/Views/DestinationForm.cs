...
namespace DEPTHCHK.Views
{
    public partial class DestinationForm : Form
    {
        ...
        private void btnEdit_Click(object sender, EventArgs e)
        {
            ...
            currentTujuan = _db.Tujuans.FirstOrDefault(t => t.KodeTujuan == KodeTujuan);
            if (currentTujuan != null)
            {
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            using (var db = new depthchkDBContext())
            {
                // Load the MobilTangki entity and its related details
                var tujuan = db.Tujuans.FirstOrDefault(t => t.KodeTujuan == kodeTujuan);
                if (tujuan != null)
                {
                    // Remove the Tujuan row
                    db.Tujuans.Remove(tujuan);
                ...
                if (openDlg.ShowDialog() == DialogResult.OK)
                {
                    using (var workbook = new XLWorkbook(openDlg.FileName))
                    {
                        ...
                        foreach (var row in rows)
                        {
                            ...
                            var existing = _db.Tujuans.Find(kodeTujuan);
                            if (existing == null)
                            {
                                // create new record
                                var t = new TblTujuan
        ...
        private void btnExport_Click(object sender, EventArgs e)
        {
            ...
            })
            {
                ...
                using (var workbook = new XLWorkbook())
                {
                    ...
                    // data
                    var tujuanlist = db.Tujuans.OrderBy(t => t.KodeTujuan).ToList();
                    int tRow = 2;
